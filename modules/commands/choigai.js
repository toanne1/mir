...
async function makeImage({ one, two }) {
    ...
    namtay_img.composite(circleOne.resize(150, 150), 56, 164).composite(circleTwo.resize(150, 150), 380, 190);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Đã quá anh ui :3",
            mentions: [{
          tag: tag,
          id: mention
        }],
