...
module.exports.run = function({ api, event, args, permssion }) {
	...
	switch (args[0]) {
		case "add": {
			...
			return api.sendMessage(`Bạn đã thêm báo lỗi mới thành công`, event.threadID, event.messageID);
		}
		case "list":
		case "all": {
			if (getData.length == 0) return api.sendMessage(`Hiện tại chưa có lỗi nào để hiển thị!`, event.threadID, event.messageID);
			var workList = "";
			getData.map(item => workList += `\n- ${item}`);
			return api.sendMessage(`Sau đây là toàn bộ lỗi: ${workList}`, event.threadID, event.messageID);
		}

		case "delete":
		case "del": {
			if (permssion !== 2) return api.sendMessage("Bạn không đủ quyền hạn để có thể sử dụng delete!", event.threadID, event.messageID);
			if (getData.length == 0) return api.sendMessage(`Hiện tại chưa có lỗi nào để có thể xóa!`, event.threadID, event.messageID);
