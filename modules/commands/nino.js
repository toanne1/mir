module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "DungUwU",
    description: "Nói chi?n z?i bot nino cute",
    commandCategory: "General",
    usages: "[câu h?i]/[on,off]",
    cooldowns: 5
};
...
module.exports.handleEvent = async ({ api, event, args, Threads }) => {
    ...
    if (nino.hasOwnProperty(threadID) && nino[threadID] == true) {
      ...
      axios.get(encodeURI(`https://adreno-api.rootandroid.repl.co/nino/get/${event.body}`)).then(res => {
            if (res.data.reply == "null" || res.data.reply == "?a nói j hong hi?u :<") {
...
module.exports.run = async ({ api, event, args, Threads }) => {
    ...
    if (!args[0]) { api.sendMessage("?a h?i j h?i di ;-;", threadID, messageID) } else {
        switch(args[0]) {
            case "on": {
                nino[threadID] = true;
                api.sendMessage("b?t ninoreply thành công!", threadID);
                break;
            }
            case "off": {
                nino[threadID] = false;
                api.sendMessage("dã t?t ninoreply", threadID);
                break;
            }
            default:
