...
async function makeImage({ one, two }) {
    ...
    let batgiam_img = await jimp.read(__root + "/phongvan.png");
    ...
    batgiam_img.resize(500, 357).composite(circleOne.resize(100, 100), 370, 50).composite(circleTwo.resize(100, 100), 200, 170);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Anh có lời gì muốn nói không ? " + tag + '\n',
            mentions: [{
          tag: tag,
          id: mention
        }],
